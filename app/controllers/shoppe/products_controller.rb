module Shoppe
  class ProductsController < Shoppe::ApplicationController
    ...
    def index
      @products = Shoppe::Product.root.includes(:translations, :stock_level_adjustments, :images, :product_categories, :variants).order(:name).group_by(&:product_category).sort_by { |cat,pro| cat.name }
    end
    ...
    def safe_params
      params[:product].permit(:name, :sku, :permalink, :description, :short_description, :weight, :price, :cost_price, :tax_rate_id, :stock_control, :data_sheet, :active, :featured, :in_the_box, :product_attributes_array => [:key, :value, :searchable, :public], :product_category_ids => [])
    end
